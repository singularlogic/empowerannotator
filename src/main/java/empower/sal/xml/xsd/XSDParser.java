			...
			if(this.parser == null) {
				...
				ErrorHandler errorHandler = new ErrorHandler() {
					...
					public void warning(SAXParseException arg0)
							throws SAXException {
						System.err.println("warning: " + arg0.getMessage());						
					}
				};
                                this.parser.setEntityResolver(new EntityResolver () {
                                    public InputSource resolveEntity(String a0, String a1) throws SAXException, IOException {
                                        System.out.println("resolve " + a0 + " " + a1);
                                            return null;
                                    }
                                });
		...
		if(edecl != null) {
		    ...
		    if(complexType != null) {
	        	...
	        	while(xstype.getBaseType() != null) {
	        		...
	        		if(xstype.getBaseType().getName().equalsIgnoreCase("anySimpleType")) break;
	        		xstype = xstype.getBaseType();
	        	}
	        	
                        String type = xstype.getName();

                        if(type.equalsIgnoreCase("QName")) type = "string";
                        
	        	System.out.println("element: " + name + " type: " + type);
	        	String namespace = edecl.getTargetNamespace();
	        	//log.debug("name: " + name + " type: " + type + " typenamespace: " + namespace);
...
/*
		    	System.out.println("simple: " + edecl.getName() + "simple:" + simpleType.getName());
		    	System.out.println("simple: " + edecl.getName() + "simplebase:" + simpleType.getSimpleBaseType().getName());
		    	System.out.println("simple: " + edecl.getName() + "base:" + simpleType.getBaseType().getName());
		    	System.out.println("simple: " + edecl.getName() + "primitive:" + simpleType.getPrimitiveType().getName());
*/
		    	...
		    	/* deprecated - annotation is loaded externally
		    		.element("annotation", "no annotation");
		    	*/		    	
		    }
		} else {
	...
	public JSONObject buildTemplateFromComplexType(JSONArray groups, String complexType) {
		Iterator<XSSchema> i = this.schemaSet.iterateSchema();
		while(i.hasNext()) {
			XSSchema s = i.next();
 			XSComplexType rootElementDecl = s.getComplexType(complexType);
			if(rootElementDecl != null) {
				return buildTemplate(groups, rootElementDecl);
			}
		}
...
//    	XSSimpleType simpleType = contentType.asSimpleType();
	...
	public JSONObject buildTemplate(JSONArray groups, XSComplexType complexType) {
		JSONObject result = new JSONObject();
		String root = complexType.getName();
		String nm = complexType.getTargetNamespace();
                String prefix = this.getPrefixForNamespace(nm);
    	...
    	XSType xstype = complexType.getBaseType();
    	while(xstype.getBaseType() != null) {
    		if(xstype.getName().equals(xstype.getBaseType().getName())) break;
    		if(xstype.getName().equalsIgnoreCase("string")) break;
    		xstype = xstype.getBaseType();
		...
		result = result.element("name", root).element("type", xstype.getName());
		// element types
		XSSimpleType simpleType = complexType.asSimpleType();
                XSContentType contentType = complexType.getContentType();
//    	XSSimpleType simpleType = contentType.asSimpleType();
		XSParticle particle = contentType.asParticle();
...
//				JSONArray elementChildren = new JSONArray();
	...
	public JSONObject getRootElementDescription() {
//		System.out.println(".....looking for roots.......");
		Iterator<XSSchema> i = schemaSet.iterateSchema();
		while(i.hasNext()) {
			XSSchema s = i.next();
			...
			while(k.hasNext()) {
				String key = k.next();
				JSONObject o = this.getElementDescription(key);
				if(o != null) return o;
			}
		}
	...
	public JSONObject getComplexTypeDescription(String type) {
		XSComplexType complexType = null;
                JSONObject result = new JSONObject();

                System.out.println(" In getComplexTypeDescription - " + type);
                
		Iterator<XSSchema> i = schemaSet.iterateSchema();
		while(i.hasNext()) {
			XSSchema s = i.next();
                        System.out.println(s.getComplexTypes());
			complexType = s.getComplexType(type);

                    System.out.println(" In getComplexTypeDescription - " + complexType);
		    ...
		    if(complexType != null) {
				...
				String name = type;
	        	// get root base type
	        	XSType xstype = complexType;
	        	System.out.println("element: " + name + " xstype: " + xstype);
	        	while(xstype.getBaseType() != null) {
	        		if(xstype.getName().equals(xstype.getBaseType().getName())) break;
	        		if(xstype.getName().equalsIgnoreCase("string")) break;
	        		xstype = xstype.getBaseType();
