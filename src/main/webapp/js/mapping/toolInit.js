...
function init(input, inputType, mapping, output, outputType) {
    initGUIPanels();
    ajaxInitMappings(input, inputType, mapping, output, outputType);
}
...
function initMappingsResponse(response) {
    ...
    inputTree.expandAll();
    showGroupElements(targetDefinition.groups[0]);
...
function initGUIPanels() {
    ...
    transformPanel.setHeader("Preview");
...
function isSupportedType(type) {
	if(type === "string" || type === "date" || type === "dateTime" || type === "dateUnion" || type === "boolean" || type ==="int" || type === "integer" || type === "decimal" || type === "float") {
		return true;
	}
...
function generateElementPanel(item, container, ident) 
{
    ...
    for(var c in item.children) {
        ...
        } else if(isSupportedType(child.type)) {
...
function generateMappingsTable(item) {
    ...
    if(item.condition != undefined) {
    	...
    	condition_type = item.condition.type;
    	condition_value = item.condition.value;
    }
    
    var warning = "";
    
    content += "<table style='width: 100%; height: 100%" + warning + "'>";
    content += "<tr>";
...
//			        	content += "<td style='vertical-align: middle; width:20px'>&nbsp = &nbsp</td>";
			        	...
			        	if(condition_value == "") {
			                content += "<td><div id='" + item.id + ".condition' target='" + item.id +"' xsdtype='"+ condition_type + "' class='mapping_value; no_mapping'>condition value</div></td>";
			        	} else {
			               ...
			               content += "<td><div id='" + item.id + ".condition' target='" + item.id +"' xsdtype='"+ condition_type + "' class='mapping_value; no_mapping'>" + condition_value + "</div></td>";
			        	}
		        	}
		        	// expand condition
	        ...
	        if(item.enumerations != null && item.enumerations.length > 0) {
		        ...
		        for(var i in item.mappings) {
		            ...
		            if(item.warning == undefined) {
				    	...
				    	content += toolButtonTD("<div id='" + item.id + index + "' style='float:right'><img src='images/img_status_warn.png' width='14px' height='14px'/></div>");
					    var tt = new YAHOO.widget.Tooltip("tt" + item.id + index, { context:"" + item.id + index, text: "Possible data loss due to type conversion", showdelay: 300 } );
		            }
				    ...
				    if(isSupportedType(item.type) || item.name.indexOf("@") === 0) {
		        	...
		        	if(type == "xpath") {
		                ...
		                content += "<div id='" + item.id + "." + index + "' xsdtype='" + item.type + "' index=\"" + index + "\"  class='" + class_value + "' style='padding: 2px'>" + value + "</div>";
		            } else if(type == "constant") {
		                value = item.mappings[i].value;
		
		                content += "<div id='" + item.id + "." + index + "' xsdtype='" + item.type + "' index=\"" + index + "\" class='" + class_value + "' style='padding: 2px'>" + value + "</div>";
		        	} else if(type == "empty") {
		                content += "<div id='" + item.id + "." + index + "' xsdtype='" + item.type + "' index=\"" + index + "\" class='" + class_value + "' style='padding: 2px'>unmapped</div>";
		        	}
		        	content += "</td>";
	        ...
	        if(item.enumerations != null && item.enumerations.length > 0) {
		        ...
		        if(item.type == undefined || isSupportedType(item.type)) {
		        	content += "<td class='elementcontent'><div id='" + item.id + ".default' xsdtype='" + item.type + "' index=\"-1\" target='" + item.id +"' class='mapping_value; no_mapping'>unmapped</div></td>";
		        } else if(item.maxOccurs != 1) {
		        	content += "<td class='elementcontent'><div id='" + item.id + ".structural' xsdtype='" + item.type + "' index=\"-1\" target='" + item.id +"' class='mapping_value'>structural</div></td>";
		        }
...
function setPanelListeners(item) {
    ...
    if(item.mappings != null && item.mappings.length > 0) {
    	...
    	var elid = id;
    	if(isSupportedType(item.type) || item.type == undefined) {
	        elid += ".default";
    	} else {
    		elid += ".structural";
    	}
