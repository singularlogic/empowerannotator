...
function generateSimpleConditionTable(condition, path) {
	...
	if(condition.relationalop != "EXISTS" && condition.relationalop != "NOTEXISTS") {
		result += valueString(condition.value, path, condition.type);
	}
	result += "</td>";
	result += "</tr>";
	result += "</table>";
...
function valueString(value, path, type) 
{
	var result = "";
	var style = "";
	if(value == undefined) {
	...
	result += "<div id='clause." + path + "' xsdtype='" + type + "' class='clause_value' style='" + style + "'>";
	result += value
	result += "</div>";
